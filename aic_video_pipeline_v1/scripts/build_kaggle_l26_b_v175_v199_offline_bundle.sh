...
OUTPUT_PATH="$(dirname "$PIPELINE_SOURCE")/aic_video_pipeline_v1_l26_b_v175_v199_offline_kaggle.zip"
STAGING_ROOT="$(mktemp -d -t aic-l26-b-v175-v199-offline-bundle.XXXXXX)"
STAGED_PIPELINE="${STAGING_ROOT}/aic_video_pipeline_v1"
TEMP_ARCHIVE="${STAGING_ROOT}/aic_video_pipeline_v1_l26_b_v175_v199_offline_kaggle.zip"
...
cleanup_staging() {
    local status=$?
    trap - EXIT
    if [[ "$STAGING_ROOT" == /tmp/aic-l26-b-v175-v199-offline-bundle.* && -d "$STAGING_ROOT" ]]; then
...
for required in \
    ...
    "${PIPELINE_SOURCE}/scripts/kaggle_run_l26_b_v175_v199_offline.sh" \
    "${PIPELINE_SOURCE}/scripts/kaggle_cell_run_l26_b_v175_v199_offline.txt"; do
    [[ -e "$required" ]] || {
...
cp -a "${PIPELINE_SOURCE}/scripts/kaggle_run_l26_b_v175_v199_offline.sh" "${STAGED_PIPELINE}/scripts/"
cp -a "${PIPELINE_SOURCE}/scripts/kaggle_cell_run_l26_b_v175_v199_offline.txt" "${STAGED_PIPELINE}/scripts/"
...
required = {
    ...
    "aic_video_pipeline_v1/scripts/kaggle_run_l26_b_v175_v199_offline.sh",
    "aic_video_pipeline_v1/scripts/kaggle_cell_run_l26_b_v175_v199_offline.txt",
...
for marker in (
    ...
    if marker not in launcher:
        raise SystemExit(f"Launcher không đúng L26_b V175-V199: {marker}")
...
echo "Đã tạo bundle offline L26_b V175-V199: $OUTPUT_PATH"
