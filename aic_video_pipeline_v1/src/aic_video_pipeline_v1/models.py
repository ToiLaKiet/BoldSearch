...
class FrameRecord:
    ...
    def from_dict(cls, value: dict[str, Any], video_id: str) -> "FrameRecord":
        allowed = {"frame_id", "frame_index", "timestamp_ms", "shot_id",
                   "preliminary_status", "mapping_status", "embedding_status",
                   "frame_path", "vector_path",
...
def validate_frame(frame: FrameRecord, *, require_final: bool = False) -> None:
    ...
    if frame.mapping_status not in {"PENDING", "EXTRACTED", "MAPPED"}:
        raise ValueError("invalid mapping_status")
    if frame.embedding_status not in {"PENDING", "EMBEDDED"}:
        raise ValueError("invalid embedding_status")
    if frame.final_status not in {"PENDING", "KEPT", "DUPLICATE"}:
        raise ValueError("invalid final_status")
    if require_final:
        if not frame.shot_id or frame.mapping_status != "MAPPED":
            raise ValueError("final frame must be mapped to a shot")
        if frame.embedding_status != "EMBEDDED" or frame.final_status != "KEPT":
            raise ValueError("final frame must be embedded and KEPT")
