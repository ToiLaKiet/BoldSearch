...
│   ├── orchestrator.py    # stage ordering and artifact contract
│   ├── services.py        # AutoShot, indexing, extraction, embedding
│   ├── models.py          # FrameRecord invariants
│   └── storage.py         # JSON/PNG/NPY persistence
├── tests/
└── data/                  # local runtime artifacts, ignored by Git
```

Các vai trò `autoshot`, `frame_indexer`, `mapping`, `fgclip_embedding` và
`similarity` trong thiết kế dưới đây được cài đặt thành các hàm cô lập trong
`services.py` và được điều phối duy nhất bởi `orchestrator.py`.
